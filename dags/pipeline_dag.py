...
with DAG(
    ...
    extractor_task = BashOperator(
        ...
        bash_command=f'cd {PROJECT_DIR} && python etl/extractor.py'
    )

    # Шаг 2: Предобработка данных
    transformer_task = BashOperator(
        task_id='preprocess_data_task',
        bash_command=f'cd {PROJECT_DIR} && python etl/transformer.py'
    )

    # Шаг 3: Обучение модели
    trainer_task = BashOperator(
        task_id='train_model_task',
        bash_command=f'cd {PROJECT_DIR} && python etl/trainer.py'
    )

    # Шаг 4: Оценка модели
    evaluator_task = BashOperator(
        task_id='evaluate_model_task',
        bash_command=f'cd {PROJECT_DIR} && python etl/evaluator.py'
    )
