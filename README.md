...
# Экзаминационное задание Кузнецов.К.И.

## Содержание
[EDA](README.md#EDA)  
[Планирование пайплайна](README.md#Планирование-пайплайна)  
[Схема пайплайна ](README.md#Схема-пайплайна )  
[Разработка ETL-компонентов](README.md#Разработка-ETL-компонентов)  
[Обоснование архитектурных решений проекта](README.md#Обоснование-архитектурных-решений-проекта)  
[Оркестрация пайплайна с помощью Airflow](README.md#Оркестрация-пайплайна-с-помощью-Airflow)  
[Структура контейнера](README.md#Структура-контейнера)  
...
### Обоснование архитектурных решений проекта

#### 1. **Модульная архитектура ETL**
**Решение:**  
Выделение отдельных компонентов:
- `extractor.py` (загрузка)
- `transformer.py` (очистка)
- `trainer.py` (обучение)
- `evaluator.py` (оценка)

**Обоснование:**
- **Принцип единой ответственности:** Каждый модуль решает одну задачу
- **Упрощение тестирования:** Независимая проверка компонентов
- **Реюзабельность:** Компоненты могут использоваться в других пайплайнах
...
**Решение:**  
Использование `config.yaml` для управления параметрами

**Обоснование:**
- **Единая точка контроля:** Все настройки в одном месте
- **Безопасность:** Конфиденциальные параметры могут выноситься в переменные окружения
- **Гибкость:** Быстрая адаптация под разные среды (dev/stage/prod)
- **Воспроизводимость:** Гарантия идентичных условий выполнения
...
**Обоснование:**
- **Жизненный цикл данных:** Четкое разделение по стадиям процесса
- **Воспроизводимость:** Сохранение сырых данных для аудита
- **Изоляция артефактов:** Предотвращение случайной перезаписи

---

#### 4. **Оркестрация через Airflow**
**Решение:**  
Управление пайплайном через `pipeline_dag.py`

**Обоснование:**
- **Отслеживаемость:** Визуализация зависимостей в UI
- **Автоматическое восстановление:** Retry-механизмы
- **Расписание:** Возможность периодического запуска (по дефолту раз в день)
- **Масштабируемость:** Параллельное выполнение задач

---

#### 5. **Контейнеризация**
**Решение:**  
Docker-развертывание с `docker-compose.yml`

**Обоснование:**
- **Консистентность сред:** Идентичное окружение на всех машинах
- **Изоляция зависимостей:** Предотвращение конфликтов библиотек
- **Портативность:** Легкий перенос между локальными и облачными средами
- **Быстрый деплой:** Стандартизированный процесс запуска

---

#### 6. **Механизмы устойчивости**
**Решение:**
- Логирование в `etl_pipeline.log`
- Retry политики в Airflow
- Проверки на каждом этапе

**Обоснование:**
- **Диагностика сбоев:** Детальные логи для анализа инцидентов
- **Автовосстановление:** 3 попытки выполнения задач
- **Защита от аномалий:** Валидация входных данных
- **Грейсфул деградация:** Продолжение работы после исправимых ошибок
...
|-----------------|-----------------|----------------------------------------|
| Обработка данных| Pandas          | Оптимально для небольших датасетов     |
| ML-библиотеки   | Scikit-learn    | Широкие возможности для классификации  |
| Сериализация    | Pickle          | Простота использования для моделей     |
| Конфигурация    | YAML            | Человекочитаемый формат                |

---

#### 8. **Принципы обработки данных**
**Ключевые решения:**
1. **Кодирование категориальных признаков:** `LabelEncoder` в трансформере
2. **Стратифицированное разбиение:** Сохранение распределения классов
3. **Feature engineering:** PCA + StandardScaler + PowerTransformer в пайплайне обучения
4. **Метрики качества:** Accuracy, Precision, Recall, F1

**Обоснование:**
- **Соответствие домену:** Для медицинских данных важна recall (минимизация ложно-негативных)
- **Воспроизводимость:** Фиксированный random_state=42
- **Сравнимость:** Стандартный набор метрик для бинарной классификации

---

#### 9. **Компромиссы архитектуры**
1. **Монолитность vs Микросервисы:**  
   Выбрана модульная монолитная архитектура как оптимальная для текущего масштаба

2. **Локальное хранение vs Облако:**  
   Локальная файловая система для простоты (возможность миграции на S3/MinIO)

3. **Полный пайплайн vs Инкрементальность:**  
   Пакетная обработка как основа (с потенциалом добавления потоковой)

---

### Эволюционный потенциал архитектуры
1. **Версионирование данных:** Интеграция с DVC
2. **Контейнеризация компонентов:** Отдельные Docker-образы для каждого этапа
3. **Сервисная архитектура:** Вынесение ML-модели в REST API
4. **Потоковая обработка:** Поддержка Kafka/Pulsar для реального времени
5. **Feature store:** Выделенное хранилище признаков

Текущая архитектура обеспечивает баланс между:
- **Простотой** разработки и поддержки
- **Устойчивостью** к сбоям
- **Готовностью к масштабированию**  
при сохранении возможности поэтапной эволюции системы.
...
Метрика recall, являющаяся для нас первичной в задаче классификации злокачественных опухолей составляет 0.95, что говорит о том, что модель упускает 5% истинно-положительных результатов и требует улучшения путем сбора данных и дообучения модели.
